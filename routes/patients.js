  ...
  async (req, res) => {
    try {
      ...
      if (!patientData.patientType || !patientData.record) {
        return res
          .status(400)
          .json({ message: "Patient type and record are required." });
      }

      // Check for existing patient based on name in the record
      const patientName =
        patientData.patientType === "ob-gyne"
      ...
      if (patientName) {
        ...
        if (existingPatient) {
          return res
            .status(400)
            .json({
              message: `Patient with name "${patientName}" already exists.`,
            });
        }
      }
      ...
      if (patientData.patientType === "ob-gyne") {
        ...
        return res
          .status(400)
          .json({ message: "Invalid patient type specified." });
      }
...
router.get("/search", authenticateToken, async (req, res) => {
  try {
    ...
    const patients = await Patient.find(searchQuery)
      .select(
        "patientId patientType patientNumber pediatricRecord.nameOfChildren pediatricRecord.nameOfMother pediatricRecord.address pediatricRecord.birthDate pediatricRecord.age obGyneRecord.patientName obGyneRecord.address obGyneRecord.birthDate obGyneRecord.age contactInfo status createdAt"
      )
      .sort({ createdAt: -1 })
...
router.get("/", authenticateToken, async (req, res) => {
  try {
    ...
    const patients = await Patient.find(query)
      .select(
        "patientId patientType patientNumber pediatricRecord.nameOfChildren pediatricRecord.nameOfMother pediatricRecord.address pediatricRecord.birthDate pediatricRecord.age obGyneRecord.patientName obGyneRecord.address obGyneRecord.birthDate obGyneRecord.age contactInfo status createdAt updatedAt"
      )
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(400)
        .json({
          message: "Error adding consultation record",
          error: error.message,
        });
    }
  }
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(400)
        .json({
          message: "Error adding immunization record",
          error: error.message,
        });
    }
  }
...
router.get("/status/:status", authenticateToken, async (req, res) => {
  try {
    ...
    for (const patient of patients) {
      ...
      patientObj.latestAppointment =
        latestAppointment[0]?.latestAppointment || null;
